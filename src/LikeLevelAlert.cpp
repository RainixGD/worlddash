...
bool LikeLevelAlert::init(int levelId) {
	...
	reqId = ReqManager::getInstance()->makeReq(ReqManager::serverLink + "likeLevel", message,
...
void LikeLevelAlert::onResponse(std::string response) {
	auto json = json::parse(response);
	if (json["ok"].as_bool()) {
		log::info("{}", json["message"].as_string());
		FLAlertLayer::create("Ok", "Level is Liked!", "ok")->show();
	}
	else {
...
void LikeLevelAlert::onError(std::string error) {
	FLAlertLayer::create("Error!", error, "ok")->show();
	FLAlertLayer::keyBackClicked();
}

void LikeLevelAlert::keyBackClicked() {
	ReqManager::getInstance()->cancelReqById(reqId);
	FLAlertLayer::keyBackClicked();
}

LikeLevelAlert* LikeLevelAlert::create(int levelId) {
	auto ret = new LikeLevelAlert();
